...
这个理解目前来自 8 篇已批准来源，还没有经过 Palantir 官方材料或一手案例交叉验证。
...
第一个 ontology 实操例子把这条路径里的中段落到了一个最小对象上：

```text
flight_alerts dataset
-> Flight Alert object type
-> properties and link to Flight
-> Assign Root Cause action
-> data lineage back to dataset / pipeline
```

它让“基于表数据的面向对象业务表达”这个理解更具体，但它仍没有展示复杂 workflow、
多步骤审批和 AI 参与行动治理。

流程挖掘案例补上了一个更接近业务决策的候选路径：

```text
SAP 数据
-> 流程对象和流程日志
-> ontology 中的销售订单项、客户、产品、生产线和信用冻结状态
-> AIP Logic 生成维持冻结 / 解除冻结建议
-> 建议和推理写回 ontology
-> Automate 触发判断函数
```

它支持“治理后的 ontology 对象模型比临时拼接数据更适合承载反复发生的业务决策”。
但它仍没有证明自动解除冻结、写回 SAP、审批、审计和回滚已经形成完整闭环。
...
- `references/source-notes/palantir-eleven-part-preface.md`
- `references/source-notes/palantir-decision-paradigm.md`
- `references/source-notes/palantir-ontology-foundation.md`
- `references/source-notes/palantir-dynamic-ontology-core.md`
- `references/source-notes/palantir-toolchain-dynamic-ontology-1.md`
- `references/source-notes/palantir-semantic-compiler-engine-1.md`
- `references/source-notes/palantir-first-ontology-practice.md`
- `references/source-notes/palantir-aip-process-mining.md`
